...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(wordOne.begin(), wordOne.end(), wordOne.begin(), ::toupper);
    std::transform(wordTwo.begin(), wordTwo.end(), wordTwo.begin(), ::toupper);
    
    auto lookupFirst = dict.find(wordOne);
    auto lookupSecond = dict.find(wordTwo);

    if (lookupFirst != dict.end() && lookupSecond != dict.end()) {
        if (lookupFirst->second == lookupSecond->second) {
            return true;
        }
    }
    return false;
