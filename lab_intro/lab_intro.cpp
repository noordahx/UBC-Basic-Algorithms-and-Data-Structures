...
PNG grayscale(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      grayvalue = adjusted_blue + adjusted_green + adjusted_red;
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double brightness = 1.0 - (sqrt((x - centerX)*(x - centerX) + (y - centerY)*(y - centerY)) * 0.005);
...
PNG ubcify(PNG image) {
  RGBAPixel* UBC_YELLOW = new RGBAPixel(247, 184, 0, 1.0);
  RGBAPixel* UBC_BLUE = new RGBAPixel(12, 35, 68, 1.0);
  
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
     RGBAPixel* pixel = image.getPixel(x, y);
  
     double distance_yellow = colordist(*pixel, *UBC_YELLOW);
     double distance_blue = colordist(*pixel, *UBC_BLUE);

     //cout << distance_yellow << " "<< distance_blue << endl;
     if (distance_yellow <= distance_blue){
      *pixel = *UBC_YELLOW;
     }
     else {
      *pixel = *UBC_BLUE;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  RGBAPixel* pureWhite = new RGBAPixel(255, 255, 255);
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      RGBAPixel* firstPixel = firstImage.getPixel(x, y);
      RGBAPixel* secondPixel = secondImage.getPixel(x, y);
      if (colordist(*secondPixel, *pureWhite) == 0){
        firstPixel->r = (firstPixel->r >= 215)? 255 : firstPixel->r+40;
        firstPixel->g = (firstPixel->g >= 215)? 255 : firstPixel->g+40;
        firstPixel->b = (firstPixel->b >= 215)? 255 : firstPixel->b+40;
      }
    }
  }
  return firstImage;
