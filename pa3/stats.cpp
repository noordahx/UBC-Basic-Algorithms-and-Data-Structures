...
/* Your code here!! */
    long redsum = getSum('r', ul, w, h);
    double red = getSumSq('r', ul, w, h) - (redsum * redsum * 1.0) / (w*h); 

    long greensum = getSum('g', ul, w, h);
    double green = getSumSq('g', ul, w, h) - (greensum * greensum * 1.0) / (w*h); 

    long bluesum = getSum('b', ul, w, h);
    double blue = getSumSq('b', ul, w, h) - (bluesum * bluesum * 1.0) / (w*h); 
