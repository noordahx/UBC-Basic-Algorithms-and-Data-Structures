...
struct BinaryTree
{
  ...
  private:
    ...
    void printLeftToRightHelper(const Node* subRoot) const;
    void invert(Node* subRoot) const;
    void paths(Node* subRoot, T arr[], int height) const;
    void printArr(T arr[], int height) const;
    int sumDistances(Node * subRoot, int depth) const;     
    void vectorize(Node * subRoot, vector<T> &result) const;
    /**
